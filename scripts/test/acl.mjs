...
if (!prov) {
  ...
  const status = () => fetch(GW + '/api/status').then(r => r.json())
  const r1 = await jpost('/api/keys', { id: 'deepseek-official', key: 'sk-acl-check' })
  await new Promise(r => setTimeout(r, 90))
  const on = ((await status()).providers || []).find(p => p.id === 'deepseek-official')?.hasKey
  const r2 = await jpost('/api/keys', { id: 'deepseek-official', key: '' })
  await new Promise(r => setTimeout(r, 90))
  const off = ((await status()).providers || []).find(p => p.id === 'deepseek-official')?.hasKey
